...
```  
  ...
  */
  String sql = """
   ...
   // populate() triggers the processing and setting of the relationship
...
3. The response from the extractor is [RelationshipMapper](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/io.github.simplejdbcmapper.core.relationship.RelationshipMapper). For each entity type it will have its corresponding results in a list. Each list will have entries **unique by ID** (ie no duplicates in each list). The extractor will handle the cases where the column to property mappings do not follow the underscore to camel case naming convention.
4. [Relationship](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/o/github/simplejdbcmapper/relationship/package-summary.html) works with the information provided by the API. It does not access the database or use SimpleJdbcMapper.
6. The collection property populated by toMany **always has to be of type 'List'**. In the above example order.orderLines has to be of type List.
7. In the example, there was only one query parameter so JdbcTemplate was used. If you have many parameters you can use NamedParameterJdbcTemplate with the framework's ResultSetExtractor (It implements Spring's org.springframework.jdbc.core.ResultSetExtractor).
...
```
 ...
 // Use JdbcTemplate with the framework extractor to execute the query and extract results
  ...
  /*
    populate  the 'product' property on OrderLine using toOne() since its a toOne relationship. As previously mentioned populate() triggers the processing.
  */
