...
2. [Relationship](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/io/github/simplejdbcmapper/relationship/Relationship.html) works with the query results and information provided by the API. It does not access the database or use SimpleJdbcMapper.
...
4. Self join tables where for example employee and manager both point to same table, you will have to map the manager as a different class and use in queries. Different classes can be mapped to the same table see [Table annotation](#annotations).
