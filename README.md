...
**New Feature Alert:** Relationships can be populated from custom queries. :sparkler:
...
```
1. The sql columns generated by [getMultiEntitySqlColumns()](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/io/github/simplejdbcmapper/core/SimpleJdbcMapper.html#resultSetExtractor%28io.github.simplejdbcmapper.core.MultiEntity%29) and the [ResultSetExtractor](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/io/github/simplejdbcmapper/core/SimpleJdbcMapper.html#resultSetExtractor%28io.github.simplejdbcmapper.core.MultiEntity%29) work together. The extractor expects columns to be exactly in a specific order, so **absolutely do not modify the sql columns string** from getMultiEntitySqlColumns().
2. [Relationship](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/io/github/simplejdbcmapper/relationship/Relationship.html) works with the information provided by the API. It does not access the database or use SimpleJdbcMapper.
3. In the example, there was only one query parameter so JdbcTemplate was used. Since ResultSetExtractor implements Spring's org.springframework.jdbc.core.ResultSetExtractor it can be used with NamedParameterJdbcTemplate/JdbcClient.
...
```
  ...
  // populate  the 'product' property on OrderLine using toOne() since its a toOne relationship. populate() method does the processing.
...
```
  ...
  """.formatted(sjm.getMultiEntitySqlColumns(multiEntity));
    ...
    List<Employee> employees = relationshipMapper.type(Employee.class).toMany(Skill.class).through(EmployeeSkill.class, "employeeId", "skillId").populate("skills").getList(Employee.class);
```
