...
[Javadoc](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/index.html) | [Website](https://spring-jdbc-crud.github.io/simplejdbcmapper) | [Demo Application](https://github.com/spring-jdbc-crud/spring-crud-with-simplejdbcmapper)
...
SimpleJdbcMapper should always be prepared in a Spring application context and given to services as a bean reference. It maintains state, for example caches insert/update sql etc.
...
Some postgres drivers for column definition 'TIMESTAMP WITH TIMEZONE' return java.sql.Types.TIMESTAMP which causes conversion failures when used with OffsetDateTime. Do the following to override it to be java.sql.Types.TIMESTAMP_WITH_TIMEZONE.
