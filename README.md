...
A simple wrapper around Spring JDBC libraries that makes CRUD operations less verbose. Use it along with Spring JdbcClient/JdbcTemplate. 
  
[Javadoc](https://spring-jdbc-crud.github.io/simplejdbcmapper/javadoc/index.html) | [Website](https://simplejdbcmapper.github.io/)
...
## Features
  ...
  3. Helper methods to get the sql for the mapped objects that can be used with Springs BeanPropertyRowMapper/SimplePropertyRowMapper. Also some methods to merge multiple results.
  ...
  6. Tested against PostgreSQL, MySQL, Oracle, SQLServer (The tests are run against these databases).
  7. No other external dependencies other than Spring JDBC libraries.
...
## Example code
 ...
 public class Product {
     /*
      ...
      @Id(type=IdType.AUTO_GENERATED) - This is for ids which are auto generated by the database. id value will 
     ...
     */
     ...
     // will map to 'product_name' column in database table.
     @Column(name="product_name")            
     private String name;                    
     
     // will map to column 'available_date' by default using camel case to underscore case convention
     @Column
...
...
 ...
 // because id type is AUTO_GENERATED, id value will be assigned on insert.
 sjm.insert(product); 
...
## Maven coordinates
 ...
 ``` 
  <dependency>
    <groupId>io.github.simplejdbcmapper</groupId>
...
Examples for different databases below. Depending on the versions of springboot/database/driver, there could be some differences to the properties.
The DataSource properties are the similar properties used to configure Spring JdbcClient or JdbcTemplate.
...
@Table(name="product", catalog="someDatabaseName", schema="someSchemaName")  // this is usually for sqlserver databases
class Product {
  ...
}
...
public SimpleJdbcMapper simpleJdbcMapper(DataSource dataSource) {
    ...
    simpleJdbcMapper.setRecordOperatorResolver(new YourImplementationOfInterfaceRecordOperatorResolver());
...
## TroubleShooting
Make sure you can connect to your database and issue a simple query using Spring JdbcClient or JdbcTemplate without the SimpleJdbcMapper.
